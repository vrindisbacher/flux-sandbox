...
#[flux_rs::assoc(fn steps_between_bool(start: Self, end: Self) -> bool )]
#[flux_rs::assoc(fn steps_between(start: Self, end: Self) -> int )]
...
trait Step {
    #[flux_rs::sig(fn(&Self[@start], &Self[@end]) -> (usize[<Self as Step>::steps_between(start, end)], Option<usize>[<Self as Step>::steps_between_bool(start, end)]))]
...
#[flux_rs::assoc(fn steps_between_bool(start: int, end: int) -> bool { start <= end } )]
#[flux_rs::assoc(fn steps_between(start: int, end: int) -> int { if start <= end { end - start } else { 0 } } )]
...
impl Step for usize {
    #[sig(fn(&usize[@start], &usize[@end]) -> (usize[<Self as Step>::steps_between(start, end)], Option<usize>[<Self as Step>::steps_between_bool(start, end)]))]
...
#[flux_rs::assoc(fn step_backward(start: int, count: int) -> int { start - count } )]
impl Step for i32 {
    #[sig(fn(&i32[@start], &i32[@end]) -> (usize[<Self as Step>::steps_between(start, end)], Option<usize>[<Self as Step>::steps_between_bool(start, end)]))]
